...
# ═══════════════════════════════════════════════════════════════════════════
# T6-T8 – Per-scenario RFI-based hub selection (all 64 scenarios, NB11-consistent)
# ═══════════════════════════════════════════════════════════════════════════
print("\n── T6-T8  Per-scenario RFI hub selection (NB11-consistent) ─────")

K_RECOMMENDED   = 6
N_SAMPLE_ROUTES = 8   # routes shown per hub — matches NB11 .groupby().head(8)
...
nb11_cols = {
    ...
    "drone_direct_feasible_approval": "approval_feasible",
...
def _mm(s, invert=False):
    """Min-max normalise a Series; invert=True → higher raw value yields lower score."""
    mn, mx = s.min(), s.max()
    if mx == mn:
...
def rfi_select(scen_id, k=K_RECOMMENDED):
    """RFI-based hub selection matching NB11's route_feasibility_index formula.

    RFI = 0.35 × norm(Ds_served)  +  0.25 × norm(feasible_count)
        + 0.20 × norm(1/demand_weighted_drone_time)  +  0.20 × norm(mean_route_constraint)

    Selects the top-K lots by their per-scenario RFI score.
    """
    ...
    # Determine airspace mode
    scen_meta = scenarios_df[scenarios_df["scenario_id"] == scen_id].iloc[0]
    strict = scen_meta["airspace_mode"] == "strict_exclusion"

    # Filter routes: only to available H3 cells
    ...
    # Feasibility filter (mirrors NB11 logic):
    #   strict mode  → require drone_direct_feasible_strict == True
    #   approval mode → require drone_direct_feasible_approval == True
    if strict and "strict_feasible" in nb11_avail.columns:
        nb11_feas = nb11_avail[nb11_avail["strict_feasible"] == True].copy()
    elif "approval_feasible" in nb11_avail.columns:
        nb11_feas = nb11_avail[nb11_avail["approval_feasible"] == True].copy()
    else:
        nb11_feas = nb11_avail.copy()

    # Compute per-lot RFI components
    lot_stats = []
    for lid, g in nb11_feas.groupby("lot_id"):
        n_feas = len(g)
        ds_sum = float(g["scen_h3_score"].sum())

        # Demand-weighted drone time (shorter = better → inverted in RFI)
        w = g["scen_h3_score"].fillna(0)
        if w.sum() > 0 and "drone_time_min" in g.columns:
            dw_time = float(np.average(
                g["drone_time_min"].fillna(g["drone_time_min"].mean()), weights=w))
        elif "drone_time_min" in g.columns:
            dw_time = float(g["drone_time_min"].mean())
        else:
            dw_time = 5.0

        mean_cs = float(g["route_constraint"].mean()) if "route_constraint" in g.columns else 0.5
        lot_stats.append({"lot_id": lid, "n_feas": n_feas,
                          "ds_sum": ds_sum, "dw_time": dw_time, "mean_cs": mean_cs})

    if not lot_stats:
        return []

    stats_df = pd.DataFrame(lot_stats).set_index("lot_id")
    stats_df["_a1"] = _mm(stats_df["ds_sum"])
    stats_df["_a2"] = _mm(stats_df["n_feas"].astype(float))
    stats_df["_a3"] = _mm(stats_df["dw_time"].fillna(999), invert=True)
    stats_df["_a4"] = _mm(stats_df["mean_cs"].fillna(0))
    stats_df["rfi"]  = (0.35 * stats_df["_a1"] + 0.25 * stats_df["_a2"]
                        + 0.20 * stats_df["_a3"] + 0.20 * stats_df["_a4"]).round(6)
    stats_df.loc[stats_df["n_feas"] == 0, "rfi"] = 0.0

    # Top-K by RFI
    top_k = stats_df.sort_values("rfi", ascending=False).head(k)

    # Build result: (lot_id, all_reachable_h3_set, rfi_score)
    result = []
    for lot_id, row in top_k.iterrows():
        lot_h3 = set(nb11_feas[nb11_feas["lot_id"] == lot_id]["h3_index"].values)
        result.append((lot_id, lot_h3, float(row["rfi"])))

    # Fallback: fill any remaining slots by parking score if fewer than K lots are feasible
    if len(result) < k:
        selected_ids = {r[0] for r in result}
        remaining = sorted(
            set(cand_meta.index) - selected_ids,
        ...
        for lid in remaining:
            if len(result) >= k:
                break
            lot_h3 = set(nb11_feas[nb11_feas["lot_id"] == lid]["h3_index"].values)
            result.append((lid, lot_h3, 0.0))

    return result


# ── Run for all 64 scenarios ──────────────────────────────────────────────
print("  Running RFI-based hub selection for 64 scenarios...")
...
for _, scen_row in scenarios_df.iterrows():
    ...
    selections = rfi_select(scen_id, K_RECOMMENDED)
    ...
    for rank, (lot_id, covered_h3, gain) in enumerate(selections, 1):
        ...
        expl = f"RFI={gain:.3f} | Ds={cover_ds:.2f} | {cover_n}셀 | RFI순위#{rank} | 경로타당성{route_fi:.0%}"
        ...
        # Assignments: top-N_SAMPLE_ROUTES H3 cells by scenario score (matches NB11 head(8))
        sample_h3 = sorted(
            covered_h3,
            key=lambda h: h3_scen_pivot.loc[h, scen_id] if h in h3_scen_pivot.index else 0,
            reverse=True
        )[:N_SAMPLE_ROUTES]

        for h3_idx_val in sample_h3:
